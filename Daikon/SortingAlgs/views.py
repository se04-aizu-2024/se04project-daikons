...
from .SortingsAlgs import bubbleSortASC, bubbleSortDESC, insertionSortASC, insertionSortDESC, selectionSortASC, selectionSortDESC
import time, re
...
def insertionsort(request):
    if request.method == "POST":
        try:
            ...
            start_time = time.time()  # Record start time
            if order == "ascending":
                sorted_array = insertionSortASC(array)
            else:
                sorted_array = insertionSortDESC(array)
            end_time = time.time()  # Record end time
            process_time = round((end_time - start_time) * 1000, 2)  # Convert to milliseconds
...
def selectionsort(request):
    if request.method == "POST":
        try:
            ...
            start_time = time.time()  # Record start time
            if order == "ascending":
                sorted_array = selectionSortASC(array)
            else:
                sorted_array = selectionSortDESC(array)
            end_time = time.time()  # Record end time
            process_time = round((end_time - start_time) * 1000, 2)  # Convert to milliseconds
